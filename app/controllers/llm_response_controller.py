...
GOOGLE_API_KEY = "AIzaSyAjG_p_DA8rSsTNUt1w4zQ_7MIZ9ADqvqk"
...
async def llm_response(chain,id,mode,language):
    ...
    if str(mode)=="dirrect_QA" and id==3:
        ...
        return {"english":diagnosis_and_treatment.replace("\n","<br>"),"translated":translate(diagnosis_and_treatment.replace("\n","<br>"),language)}
    if str(mode)=="dirrect_QA" and id>3:
        qa_text =  str(QA())
    ...
    if id==13:
        ...
        print("h&P")
        return {"english":str(report['result']).replace("\n","<br>"),"translated":translate(str(report['result']).replace("\n","<br>"),language)}
